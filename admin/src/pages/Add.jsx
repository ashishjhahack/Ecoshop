...
const Add = ({token}) => {
  const [image1, setImage1] = useState(false);    // you can also use only one useState for this variables by array-object format
  ...
  const onSubmitHandler = async(e) => {
    e.preventDefault();
    try {
      const formData = new FormData();       // add this product in a formdata
      ...
      image1 && formData.append("image1", image1);        // if image is available then append the image otherwise not
      image2 && formData.append("image2", image2);
      image3 && formData.append("image3", image3);
      image4 && formData.append("image4", image4);

      // now we send this formData using api(axios) in our bakcend
      ...
      <div>
        ...
        <div className='flex gap-2'>
          <label htmlFor="image1">
            {/* by URL.createeObject we also able to display the selected image  */}
