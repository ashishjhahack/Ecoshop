...
const PlaceOrder = () => {
  ...
  const onChangeHandler = (event) => {
    ...
    setFormData(data => ({ ...data, [name]: value }))    // update this for multiple input
  }
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      let orderData = {
        address: formData,   // ✅ Should exist
        items: orderItems,
        amount: getCartAmount() + Number(delivery_fee),
        paymentMethod: method,   // ✅ REQUIRED in schema
        date: Date.now()         // ✅ REQUIRED in schema
      };
